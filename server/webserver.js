...
mongoose.connect(
 ...
 instance.delete("/api/role", function(request, response){
  ...
  instance.get("/api/user", function(request, response){
    ...
    instance.get("/api/user/", function(request, response){
      var tokenReceived = request.headers.authorization.split(" ")[1];
      console.log("tokn", tokenReceived);
      ...
      jwt.verify(tokenReceived, instance.get("jwtSecret"), function(err, decoded){
        if(err){
          ...
          request.decoded = decoded;
            ...
            User.getUsersSearch(request, response)
...
instance.get("/api/personalinfo/byusernametemp/:userName", function(request, response){
  ...
  jwt.verify(tokenReceived, instance.get("jwtSecret"), function(err, decoded){
    if(err){
      ...
      request.decoded = decoded;  
      PersonalInfo.getPersonalInfoByUserNameTemp(request, response)
